  ...
  return withTryJudge({
    judge(value): string {
      ...
      if (opts?.version === '4' && !ipv4Regex.test(value)) {
        throw new JudgmentError([{ message: 'Invalid IPv4 address', path: '' }]);
      }

      if (opts?.version === '6' && !ipv6Regex.test(value)) {
        throw new JudgmentError([{ message: 'Invalid IPv6 address', path: '' }]);
      }

      if (!ipv4Regex.test(value) && !ipv6Regex.test(value)) {
        throw new JudgmentError([{ message: 'Invalid IP address', path: '' }]);
      }
